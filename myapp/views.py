...
def reset_password(request):
    ...
    if request.method == "POST":
        ...
        if len(new_password) < 8:
            return render(request, 'reset_password.html', {'error': 'La contraseña debe tener 8 caracteres.'})
        
        # Requisitos de complejidad (usando regex)
        # Al menos 2 letras mayúsculas
        if len(re.findall(r'[A-Z]', new_password)) < 2:
            return render(request, 'reset_password.html', {'error': 'Incluir al menos 2 letras mayúsculas.'})
        
        # Al menos 2 letras minúsculas
        if len(re.findall(r'[a-z]', new_password)) < 2:
            return render(request, 'reset_password.html', {'error': 'Incluir al menos 2 letras minúsculas.'})
        
        # Al menos 3 números
        if len(re.findall(r'\d', new_password)) < 3:
            return render(request, 'reset_password.html', {'error': 'Incluir al menos 3 números.'})
        
        # Al menos 1 carácter especial (que no sea letra, número o espacio)
        if not re.search(r'[^\w\s]', new_password):
            return render(request, 'reset_password.html', {'error': 'Incluir al menos 1 carácter especial.'})
